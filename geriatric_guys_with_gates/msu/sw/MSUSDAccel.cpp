...
void MSUSDAccel::init(MSU *_msu, Squarer *_squarer) {
    ...
    int words_in  = msu->mod_len / MSU_WORD_LEN;
    int words_out = ceil( ( (WORD_LEN+1) * NUM_ELEMENTS ) / MSU_WORD_LEN );
    msu_words_in  = (T_LEN/MSU_WORD_LEN*2 + words_in);
    msu_words_out = (T_LEN/MSU_WORD_LEN + words_out);
