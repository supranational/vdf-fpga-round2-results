...
public:
    SquarerOzturk(uint64_t _mod_len, mpz_t _modulus)
        ...
        // These must agree with calculation of localparams AXI_IN_COUNT AXI_OUT_COUNT in msu.sv
        words_in  = _mod_len / MSU_WORD_LEN;
        words_out = ceil( ( (WORD_LEN+1) * NUM_ELEMENTS ) / MSU_WORD_LEN );
    }
    
    // this must equal localparam AXI_IN_COUNT in msu.sv
    virtual uint64_t msu_words_in() {
        return(T_LEN/MSU_WORD_LEN*2 + words_in);
    }
    
    // this must equal localparam AXI_OUT_COUNT in msu.sv
    ...
    virtual void unpack(mpz_t sq_out, uint64_t *t_final, mpz_t msu_out,
        ...
        *t_final = mpz_get_ui(msu_out);
        ...
        reduce_polynomial(sq_out, msu_out, word_len, (word_len+1));
    }        
