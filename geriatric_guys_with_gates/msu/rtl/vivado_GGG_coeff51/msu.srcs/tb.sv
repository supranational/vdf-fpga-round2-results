...
module tb();
   ...
   // Ozturk parameters
   localparam integer WORD_LEN         = 50;
   localparam integer BIT_LEN          = 51;
   ...
   initial begin
      ...
      while(1) begin
         ...
         sq_out_reducing = 0;
         for(i = 0; i < SQ_OUT_BITS / BIT_LEN; i++) begin
            if(i > 0) begin
               sq_out_reducing <<= WORD_LEN;
               sq_out_actual   <<= BIT_LEN;
            end
            sq_out_reducing += sq_out_actual[SQ_OUT_BITS-BIT_LEN +: BIT_LEN];
         end
